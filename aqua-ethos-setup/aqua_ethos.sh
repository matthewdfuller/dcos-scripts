...
function createLabelIfDoesNotExist {
    ...
    if [ -n "$RESPONSE_LABEL" ]; then
        ...
        makePost settings/labels "{\"name\":\"$CREATE_LABEL\"}"        
...
function addBatchInstallTokenWithLabel {
    ...
    if !(tokenHasLabel "$ADD_TOKEN_NAME" "$SET_TOKEN_LABEL"); then
        ...
        TOKEN_PAYLOAD='{"logicalname":"'$ADD_TOKEN_NAME'","token":"'$ADD_TOKEN_VALUE'","description":"Batch install for production hosts.","enforce":true,"allowed_labels":["'$SET_TOKEN_LABEL'"],"allowed_registries":["Docker Hub"],"gateways":"'$(getGatewayId)'"}'
        log "$TOKEN_PAYLOAD"
